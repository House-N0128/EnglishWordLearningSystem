...
public interface WordLearningRecordMapper {
    ...
    @Select("SELECT * FROM t_word_learning_record")
    List<WordLearningRecord> findAll();
    ...
    @Select("SELECT COUNT(DISTINCT userId) FROM t_word_learning_record WHERE DATE(recordCreateTime) = CURDATE()")
    ...
    @Select("SELECT r.learnedWordBookId AS wordBookId, wb.wordBookName AS wordBookName, " +
            "(SELECT COUNT(*) FROM t_word_book_ref WHERE wordBookId = r.learnedWordBookId) AS totalWords, " +
